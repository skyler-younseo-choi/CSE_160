...
def test_common_friends_map(practice_graph, rj):
    assert_equals({'D': 2, 'F': 1},
                  sn.common_friends_map(practice_graph, "A"))
    assert_equals({'Benvolio': 1, 'Capulet': 2, 'Friar Laurence': 1,
                   'Juliet': 1, 'Montague': 2},
                  sn.common_friends_map(rj, "Mercutio"))
...
def test_recs_by_influence(rj):
    assert_equals(['Capulet', 'Montague', 'Benvolio',
                   'Friar Laurence', 'Juliet'],
                  sn.recs_by_influence(rj, "Mercutio"))
